...
/**
* Converts a ip to string representation (used by the viewer/console)
 * TODO: deprecated, only ipv4 support
 */
...
/**
 * Converts an ip string to a long (used by the viewer/console)
 * TODO: deprecated, only ipv4 support
 */
  ...
  if(u && cconf) {
    ...
    while(!locked) {
      ...
      if(sconf->qos_cc_forwardedfor) {
        ...
        if(forwardedfor) {
          new.ip = qos_inet_addr(forwardedfor);
          if(new.ip == 0) {
...
static void qos_logger_cc(request_rec *r, qos_srv_config *sconf, qs_req_ctx *rctx) {
  if(sconf->has_qos_cc) {
    ...
    if(sconf->qos_cc_forwardedfor) {
      ...
      if(forwardedfor) {
        nef.ip = qos_inet_addr(forwardedfor);
        if(nef.ip == 0) {
          if(apr_table_get(r->notes, "QOS_LOG_PFX069") == NULL) {
            ap_log_rerror(APLOG_MARK, APLOG_NOERRNO|APLOG_ERR, 0, r,
