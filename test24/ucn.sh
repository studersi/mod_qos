...
../httpd/httpd -d `pwd` -f conf/ucn.conf -D BASICAUTH 2>/dev/null 1>/dev/null
waitApache
./run.sh -seT scripts/UCN_QS_ClientIpFromHeader_basicAuth.htt
ERRORS=`expr $ERRORS + $?`
./ctl.sh stop 2>/dev/null 1>/dev/null

../httpd/httpd -d `pwd` -f conf/ucn.conf -D QS_SrvMaxConn_Only 2>/dev/null 1>/dev/null
waitApache
./run.sh -seT scripts/UCN_QS_SrvMaxConn_Only.htt
ERRORS=`expr $ERRORS + $?`
./ctl.sh stop 2>/dev/null 1>/dev/null

../httpd/httpd -d `pwd` -f conf/ucn.conf -D QS_ClientGeoCountryPriv 2>/dev/null 1>/dev/null
waitApache
./run.sh -seT scripts/UCN_QS_ClientGeoCountryPriv.htt
ERRORS=`expr $ERRORS + $?`
./ctl.sh stop 2>/dev/null 1>/dev/null

../httpd/httpd -d `pwd` -f conf/ucn.conf -D QS_ClientGeoCountryPriv_excludeUnknown 2>/dev/null 1>/dev/null
waitApache
./run.sh -seT scripts/UCN_QS_ClientGeoCountryPriv_excludeUnknown.htt
ERRORS=`expr $ERRORS + $?`
./ctl.sh stop 2>/dev/null 1>/dev/null
